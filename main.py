...
sheet=cliente.open_by_url('https://docs.google.com/spreadsheets/d/1Pf9OLpWmrmVtIBjQyxxZEwnFmHrf8jgeZ6kB9z2_F9g/edit#gid=0').get_worksheet(0) # https://docs.google.com/spreadsheets/d/1PhJXFOKdEAjcILQCDyJ-couaDM6EWBUXM1GVh-3gZWM/edit#gid=96577098
...
d_agend=[]
h_agend=[]
horas=['08:00','08:30','09:00','09:30','10:00','10:30','11:00','11:30','12:00','12:30','13:00','13:30','14:00','14:30','15:00','15:30','16:00','16:30','17:00','17:30','18:00','18:30','19:00','19:30']
...
status=['','Todas Ativas','Indeferido','OS Aberta','Pendente de Material','Pendente Solicitante','Pendente Outros','Atendida','Cancelada','Atendida Parcialmente','Material Solicitado','Ignorar','Observação','Reitoria','Material Disponível','Agendamento','Agendado','Em Andamento','Programada']
status_todos = ['','OS Aberta','Pendente de Material','Pendente Solicitante','Pendente Outros','Material Solicitado','Observação','Material Disponível','Agendamento','Agendado','Em Andamento','Programada']
if (pg=='Edição individual'):
    ...
    for dic in df.index:
        ...
        if filtra_os!='':
            if df['Status'][dic] in filtrando and df['Área de Manutenção'][dic] != '' and str(df['Ordem de Serviço'][dic]) == str(filtra_os):
    ...
    if len(n_solicitacao)>1 and (filtra_os!=''):
        st.markdown(alerta + f'<Strong><i>Foram encontradas {len(n_solicitacao)} Ordens de Serviço com este mesmo número, selecione abaixo a solicitação correspondente:</i></Strong></p>',unsafe_allow_html=True)
    ...
    if (len(n_solicitacao)>0):
        ...
        with st.form(key='my_form'):
            ...
            with st.expander("Agendamento"):
                ...
                if (d_agend[n] != ''):
                    ...
                    st.markdown(alerta + '<b>OS sem agendamento registrado ou com data de agendamento anterior a hoje!</b></p>', unsafe_allow_html=True)
                d = d.replace('/', '-')
        ...
        if (botao==True and s==a):
            if (sheet.cell(celula.row,20).value==n_solicitacao[n] and sheet.cell(celula.row,20).value!=0 and sheet.cell(celula.row,20).value!=''):
                with st.spinner('Registrando dados...Aguarde!'):
                    ...
                    data_agend=str(data_agendamento.year) + '-' + str(data_agendamento.month) + '-' + str(data_agendamento.day)
    ...
    for dic in df.index:
        ...
        #print(str(filtra_os))
        if filtra_os!='':
            if df['Status'][dic] in filtrando and df['Área de Manutenção'][dic] != '' and (str(df['Ordem de Serviço'][dic]) in filtra_os) and (str(df['Ordem de Serviço'][dic])!='') and (str(df['Ordem de Serviço'][dic])!=0):
                #print(df['Código da UFT'][dic])
                n_solicitacao.append(df['Código da UFT'][dic])
                nome.append(df['Nome do solicitante'][dic])
                telefone.append(df['Telefone'][dic])
    ...
    #    valor = dados1['Ordem de Serviço'].value_counts()
        if int(repetido)>1:
            st.markdown(alerta + f'<Strong><i>Foram encontradas Ordens de Serviço com números repetidos, exclua da lista a solicitação que não for correspondente a que queira editar</i></Strong></p>',unsafe_allow_html=True)
            repeticao = 1
            break
        ...
        if (botao==True and s==a):
            with st.spinner('Registrando dados...Aguarde!'):
                for selecionado_i in selecionado:
                    ...
                    # print(repeticao)
                    # print(selecionado_i)
                    if (sheet.cell(celula.row, 20).value == selecionado_i and sheet.cell(celula.row, 20).value != '' and repeticao == 0):
                        efetuado = 1
                        sheet.update_acell('W' + str(celula.row), status)  # Status
                        #sheet.update_acell('R' + str(celula.row), '')  # apagar Sim para enviar e-mail
                        if (obsemail!=''):
    ...
    with st.form(key='form1'):
        ...
        valor= list(dict.fromkeys(valor)) #removendo valores duplicados
        valor=sorted(valor) #ordenando lista de string
        filtro_os=col2.selectbox('Filtrar por Ordem de Serviço:',valor)
        if (filtro_os!=''):
            if (len(filtrar)>0):
        ...
        valor= list(dict.fromkeys(valor)) #removendo valores duplicados
        valor=sorted(valor) #ordenando lista de string
        filtro_solicitante=col3.selectbox('Filtrar por Nome do Solicitante:',valor)
        ...
        valor = list(dict.fromkeys(valor))  # removendo valores duplicados
        valor = sorted(valor)  # ordenando lista de string
        filtro_status = col4.selectbox('Filtrar por Status:', valor)
        if (filtro_status!=''):
            if (len(filtrar)>0):
        ...
        valor = list(dict.fromkeys(valor)) #removendo valores duplicados
        valor=sorted(valor) #ordenando lista de string
        filtro_predio=col5.selectbox('Filtrar por Prédio:',valor)
        if (filtro_predio!=''):
            if (len(filtrar)>0):
        ...
        valor = list(dict.fromkeys(valor))  # removendo valores duplicados
        valor = sorted(valor)  # ordenando lista de string
        filtro_sala = col6.selectbox('Filtrar por Sala/Local:', valor)
        if (filtro_sala!=''):
            if (len(filtrar)>0):
